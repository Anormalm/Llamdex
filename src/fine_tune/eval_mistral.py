...
@torch.no_grad()
def evaluate_mistral(mistral_models_path, model_name, dataset_type, system_prompt, model_state_dict_dir, batch_size,
    ...
    if model_state_dict_dir is None:
        if dp_type is None:
            if not load_lora_only:
                model_state_dict_dir = f"model/llm/exp-lora-real_mistral_{dataset_type}/model_final.pt"
            else:
                model_state_dict_dir = f"model/llm/exp-lora-real_mistral_{dataset_type}"
        else:
            if not load_lora_only:
                model_state_dict_dir = f"model/llm/exp-lora-syn_mistral_{dataset_type}_{dp_type}_{epsilon}/model_final.pt"
            else:
                model_state_dict_dir = f"model/llm/exp-lora-syn_mistral_{dataset_type}_{dp_type}_{epsilon}"

    if output_file is None:
        os.makedirs(f"data/{dataset_type}/res", exist_ok=True)
        output_file = f"data/{dataset_type}/res/fine_tune_mistral_test.csv"
...
if __name__ == "__main__":
    ...
    evaluate_mistral(args.mistral_models_path, args.model_name, args.dataset, args.system_prompt,
