...
def fine_tune_llama(llama_models_path: str, model_name: str, dataset: str, save_dir: Optional[str] = None,
    ...
    if save_dir is None:
        if dp_type is None:
            save_dir = f"model/llm/exp-lora-real_llama_{dataset}"
        else:
            save_dir = f"model/llm/exp-lora-syn_llama_{dataset}_{dp_type}_{epsilon}"

    if accelerator.is_local_main_process:
        writer = SummaryWriter(log_dir=f"runs/fine_tune_llama_{dataset}")
    ...
    if dp_type is None:
        ...
        if dp_type not in ['dp-wgan', 'pate-gan', 'prompt-pate']:
...
if __name__ == '__main__':
    ...
    fine_tune_llama(args.llama_models_path, args.model_name, args.dataset, args.save_dir, args.batch_size,
