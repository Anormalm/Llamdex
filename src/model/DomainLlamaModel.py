...
from .util import SimpleMLP, SwiGLU

logger = logging.get_logger(__name__)

class DomainLlamaDecoderLayer(nn.Module):
    def __init__(self, layer: LlamaDecoderLayer, maps_to_expert_emb=None):
        super().__init__()
    ...
    def forward(
        ...
        if self.num_experts > 0:
            ...
            hidden_states_right = self.input_layernorm(hidden_states_right)
...
class DomainLlamaModel(LlamaModel):
    def __init__(self, model: LlamaModel):
        for attr_name, attr_value in model.__dict__.items():
            setattr(self, attr_name, attr_value)

        for i in range(len(self.layers)):
            layer = DomainLlamaDecoderLayer(self.layers[i])
            self.layers[i] = layer
...
class DomainLlamaForCausalLM(LlamaForCausalLM):
    def __init__(self, config, tokenizer=None, tuned_lens=None):
        super().__init__(config)
        self.config = config
        self.num_tokens = 0
        self.tokenizer = tokenizer  # tokenizer for expert embedding translation

    def add_expert_(self, expert: DomainExpert, layer_id: int | list[int] = None, copy=True):
        """
    ...
    def replace_expert_(self, expert_dir):
        """
        ...
        for i in range(len(self.model.layers)):
            layer = self.model.layers[i]
            if isinstance(layer, DomainLlamaDecoderLayer):
    ...
    def from_pretrained_llama(
        ...
        **kwargs,
        ...
        """
        Load a model from a pretrained llama model without domain experts.
        Args:
            pretrained_model_name_or_path: path to the pretrained model or model ID of llama model
        ...
        Returns: DomainLlamaForCausalLM model with pretrained weights from LlamaForCausalLM model
        """
        ...
        # wrap model with DomainLlamaModel
        model.model = DomainLlamaModel(model.model)

        # # initialize tuned lens
        # model.tuned_lens = TunedLens.from_model_and_pretrained(model)
        # model.tuned_lens = model.tuned_lens.to(model.device).to(model.dtype)
        ...
        for i in range(len(model.model.layers)):
            model.model.layers[i] = DomainLlamaDecoderLayer(model.model.layers[i])

        return model

    # override the from_pretrained method to initialize the model with domain experts
    @classmethod
    def from_pretrained(cls, dataset_name: str, base_model_root="model/", model_id="meta-llama/Llama-2-7b-chat-hf",
        ...
        if model_state_dict_path is None:
            model_state_dict_path = os.path.join(llm_root_dir, f"domain_llama_{dataset_name}/model_final.pth")

        # Load the model
        base_model = cls.from_pretrained_llama(model_id, cache_dir=base_model_root, torch_dtype=torch.bfloat16)
    ...
    def forward(
        ...
        random_pad = torch.normal(0, 1, (input_ids.shape[0], self.num_tokens, inputs_embeds.shape[-1])).to(input_ids.device).to(torch.bfloat16)
        inputs_embeds = torch.cat((inputs_embeds, random_pad), dim=1)
