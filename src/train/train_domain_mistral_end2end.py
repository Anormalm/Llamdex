...
def train_domain_mistral(mistral_models_path: str, model_name: str, dataset_type: str, expert_dir: str, encoder_model: str, system_prompt: str,
                         save_dir: Optional[str] = None, ffn_hidden_size: int = 2048, num_tokens: int = 10, num_heads: int = 8, dropout: float = 0.0, use_norm: bool = True,
                         expert_input_size: int = 105, expert_output_size: int = 1,
                         max_new_tokens: int = 3000, batch_size: int = 64, gradient_accumulation_steps: int = 8, num_epochs: int = 2, save_period: int = 300,
                         learning_rate: float = 1e-4, mapping_hidden_size=64, layer_to_add=None, modeling_loss_ratio=0.1, warmup_steps=500, tune_lora=False, no_checkpoint=False):
    """
    ...
    embed_pad = model.model.embed_tokens(torch.tensor([tokenizer.pad_token_id]))[0]
    embed_yes = model.model.embed_tokens(torch.tensor(yes_token_list))[0]
    embed_no = model.model.embed_tokens(torch.tensor(no_token_list))[0]
    ...
    dataset = get_dataset(dataset_type, data_file, tokenizer=tokenizer, dataset_json=dataset_json, system_prompt=system_prompt)
        ...
        for index, batch in tqdm(enumerate(training_dataloader), total=len(training_dataloader),
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    ...
                    loss = modeling_loss + auxiliary_loss * auxiliary_loss_ratio
                    # loss = auxiliary_loss
                    ...
                    # if accelerator.is_local_main_process:
                    #     wandb.log({"auxiliary_loss": loss.item()})
...
if __name__ == '__main__':
    ...
    parser.add_argument("--system_prompt", type=str, default="Respond the user's question in only one word: Yes or No.", help="System prompt for the model")
    ...
    # parser.add_argument('-g', '--gpu', type=int, nargs='*', default=[0], help='GPU indices to use')

    args = parser.parse_args()

    # os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(map(str, args.gpu))

    train_domain_mistral(args.mistral_models_path, args.model_name, args.dataset, args.expert_dir, args.encoder_model, args.system_prompt, args.save_dir,
                         args.ffn_hidden_size, args.num_tokens, args.num_heads, args.dropout, args.use_norm, args.expert_input_size, args.expert_output_size,
                         args.max_new_tokens, args.batch_size, args.gradient_accumulation_steps, args.num_epochs, args.save_period,
                         args.learning_rate, args.mapping_hidden_size, args.layer, args.modeling_loss_ratio, args.warmup_steps, args.tune_lora, args.no_checkpoint)
